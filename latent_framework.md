...
Story は、神話やキャッチコピー、憲章、企業 web ページの「設立の目的」のような形で観測される。
...
### 語りの状態を示す補助ラベル – Product / Marionette / Legacy
...
例えば佇まいや行動、オーラなどから伝わる Story と、それが伝わる Product を同じ扱いにする。
...
Marionette だから価値が低いわけではなく、Blues（衝動）が切断されやすい分、Economy と Force による支えを強く要する。

#### 🏺 Legacy（遺物）

当初の語り手が消えた、もしくは入れ替わり、語りの中身が空洞化したまま形式だけが残存する状態。

- 博物館、美術館の展示物。蒙古襲来絵巻やシスティナ礼拝堂の天井画のような、改修があるものを含む
...
- カーゴカルト的儀式
- 内容や CM の作り方を少しづつ変えるロングセラー商品。ポカリスウェットや Apple 社製品など

これらは、すでに観測されないもの、観測されて意味が復活したもの、変容したものを含む。
なので、必要であればさらに 3 分類するとよい。

- 🗑️ Junk（残骸）：Story が失われ、現状では機能していない物体、あるいは、それを排除した後に残った空間
- 🖋️ Repurpose（転用）：最初の Story をあえて無視し、新たな Story を載せて動き出した物体
- 🏛️ Heritage（遺産）：Reframe を重ねつつ、博物館・地域社会などで Story が保全されている物体
...
- Legacyに資源と新たな語りが注がれると再活性化する。

これらの区分を併用することで、「いま観測している語り」がどの温度帯にあるのかを一語で共有でき、
Story・Economy・Force のどこを強化すべきかを判断しやすくなる。
